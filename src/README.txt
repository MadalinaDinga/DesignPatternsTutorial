...
----------------------------------------------------------------
     ...
     Decorator        |       Builder
attach/add features   |  create different representations
...
SINGLETON( creational) > NO PARAMETERS
    - guarantee that there is only one resource created
    ...
    - private constructor
    - pitfalls:
        - overused => slow app down
        - difficult to unit test( private constructors, private member vars)

----------------------------------------------------------------

       Singleton          |        Factory
returns the same instance | retutns various instances
one constructor( no args) | multiple constructors( diff params)
